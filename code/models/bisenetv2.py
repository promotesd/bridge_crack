...
class BiSeNetV2(nn.Module):
    ...
    def load_pretrain(self):
        ...
        backbone_path = os.path.join(this_dir, '..', '..', 'model_pth', 'backbone_v2.pth')

        state = torch.load(backbone_path, map_location='cuda',weights_only=True)
        # 逐子模块加载
...
# # ===== 以下演示代码仅用于测试推理，不修改网络结构 =====
# if __name__ == "__main__":
#     # 如果 batch size=1 并且最后空间分辨率很小，会在 train() 模式下触发 BN 的统计量不足错误。
#     # 故这里用 eval() + no_grad() 进行推理演示。
#     model = BiSeNetV2(n_classes=19, aux_mode='train')
#     model.eval()  # 切换到推理模式
